...
export function RegisterFarmingSpells(events: TSEvents) {
    ...
    //harvesting crop
    TAG('player-housing-mod', 'farming-crop-final').forEach(x => {
    ...
    //fertilizer
    TAG('player-housing-mod', 'farming-fertilizer-spell').forEach(x => {
...
function spellScriptsForPlacement(events: TSEvents) {
    TAG('player-housing-mod', 'farming-crop-spell').forEach(x => {
        events.Spell.OnCheckCast(x, (spell, result) => {
            let player = spell.GetCaster().ToPlayer();
            if (player.IsNull())
                return
            ...
            crop.Setup(spell.GetTargetDest(), player.GetO(), spell.GetSpellInfo().GetPriority(), 10.0)
            crop.MarkDirty();
            crop.Spawn(player)
        })
    })

    TAG('player-housing-mod', 'farming-gob-spell').forEach(x => {
        events.Spell.OnCheckCast(x, (spell, result) => {
            let player = spell.GetCaster().ToPlayer();
            if (player.IsNull())
                return
    ...
    TAG('player-housing-mod', 'farming-creature-spell').forEach(x => {
        events.Spell.OnCheckCast(x, (spell, result) => {
            let player = spell.GetCaster().ToPlayer();
            if (player.IsNull())
                return
