...
export const statChoices: number[][][] = [//data here is filled in datascripts
    <number[][]>[//primaries
        <number[]>[//str group
        ],
        <number[]>[//agi group
...
export function fillStatChoices() {
    ...
    while (q.GetRow()) {
        ...
        statChoices[0][q.GetUInt32(0)] = arrFill
        arrFill = <TSArray<number>>[]
        q.GetString(2).split(",").forEach((v, i, arr) => {
            arrFill.push(ToUInt32(v))
        })
        statChoices[1][q.GetUInt32(0)] = arrFill
    }
}
...
export function classIDToStatType(classID: number, seed: number): number {
    switch (classID) {
        case 1:
        case 6:
            return 0
        ...
        case 9:
            return 2
        case 2:
            return getRandNumberWithSeed(seed, 2) == 0 ? 0 : 1
        case 7:
        case 11:
            return getRandNumberWithSeed(seed, 2) + 1
        default:
            return 0
    }
}